...
class QCLIService:
    ...
    def chat(self, message: str, context: str = "", work_directory: str = None) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"使用动态超时: {dynamic_timeout}秒 (上下文长度: {len(context)})")
            ...
            # 使用实际的动态超时时间
            actual_timeout = get_timeout_for_request(bool(context), len(context))
            error_msg = f"Q CLI调用超时（{actual_timeout}秒）。AI处理复杂问题需要较长时间，请尝试简化问题或稍后重试。"
            logger.error(error_msg)
            raise RuntimeError(error_msg)
    ...
    def _prepare_message(self, message: str, context: str = "") -> str:
        ...
        参数:
            message: 用户消息
            context: 对话上下文（保留用于日志，但不发送给Q CLI）
